...
def command_start(update, context: CallbackContext):
    if update.effective_chat.type == "private":
        ...
        addme = InlineKeyboardButton(text="🧚 Məni qrupa əlavə edin!", url="https://t.me/sozoyunxbot?startgroup=a")
        oyun = InlineKeyboardButton(text="👾 Oyun Qrupumuz", url="https://t.me/SozOyunBotChat")
        admin = InlineKeyboardButton(text="👨🏻‍💻 Sahib", url="https://t.me/TheZahid")

        keyboard = [[addme],[oyun],[admin]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        update.message.reply_text('Salam, Dostum!\n\nMən qrup oyun botuyam,məni qruplarınıza əlavə edərək, dostlarınızla bərabər oyun oynayıb, əyləncəli vaxt keçirə bilərsiniz 🤠\n\nŞəxsi mesajlarda oyun oynamaq olmur,zəhmət olmasa qripa əlavə et!\n\n Şikayət vəya Təklif üçün: @TheZahid', reply_to_message_id=True, reply_markup=reply_markup)
    else:
        ...
        update.message.reply_text('Söz Oyunu Başladı, Hərkəsə Uğurlar✨'.format(username), reply_to_message_id=True)
...
def set_master(update, context):
    ...
    update.message.reply_text('[{}](tg://user?id={}) sözü başa salır 🧐\n\nSözü ilk tapan, digər turda aparıcı rolunda davam edəcək. Diqqətiniz burada olsun!'.format(username,user_id), reply_to_message_id=True, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
...
def command_master(update: Update, context):
    ...
    if not game.is_master_time_left():
        update.message.reply_text('Oyunda aparıcı olmaq üçün {} saniyə qalıb, tələsin 😉'.format(game.get_master_time_left()),
